...
# ╔═╡ 923a0a50-a599-11ed-21df-75c3ceb56c42
begin
	using Markdown
	using Graphs
	using LaTeXStrings
...
# ╔═╡ 2198dec6-420b-4b56-b7d5-16be639d1a33
using MakieTeX, CairoMakie

# ╔═╡ cb05e310-481a-4c29-a159-424ef8cf579d
...
# ╔═╡ 395eeaf1-a1a5-44cb-a069-19bc83e6e9b1
latexNodeNames = latexstring.(nodeNames)

# ╔═╡ a869b18b-d1b1-4c86-a289-51dac8da1b3c
begin
	fig, ax, p = graphplot(g; arrow_shift=0.6, arrow_size=20, curve_distance=-.5, curve_distance_usage=true)
	hidespines!(ax)
	hidexdecorations!(ax)
	hideydecorations!(ax)
	p.node_size = Vec( 100, 100 )
	p.node_color = "#9a02d6d5"
	p.nlabels = latexNodeNames
	p.nlabels_fontsize = 100
	# fig.scene.backgroundcolor = RGBAf(125/255, 5/255, 172/255, 0.2)
	ax.backgroundcolor = RGBAf(125/255, 5/255, 172/255, 0.2)
	ax.aspect = DataAspect()
	resize!(fig.scene, (0, 1300))
	colsize!(fig.layout, 1, Aspect(1, 1))
	resize_to_layout!(fig)
end

# ╔═╡ 3aa4b4a1-77a5-4f7c-b2d9-90bb72a44ed2
begin
	global idx = 1
	for (x,y) in p[:node_pos][]
		MakieTeX.LTeX(fig[1,1], latexNodeNames[idx])
		global idx += 1
	end
end

# ╔═╡ 3adfd6c4-0a2b-4850-a0a8-c98c8f490b93
fig

# ╔═╡ 3c3ad566-7a7e-4436-b83e-3f2803f19d67
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "2e09f68b142208d9e81e51520253cd8c1d7ef9e8"
